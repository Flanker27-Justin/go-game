...
const VCT_MAX_PLIES = 10;

const VCT_NODE_LIMIT = 5000;

const VCT_TIME_BUDGET_MS = 300;
...
/* ---------------- 二、引擎状态 ---------------- */
/* ⚠ 本段由 work/build-engine.js 从 engine/gomoku-ai.js 的“二、引擎状态”段抽取后
 *   回填到下面的占位符。**模板这里不要再写任何状态声明、也不要在占位符之后再写
 *   说明性注释**——两者都会被搬进模块，造成重复声明或产物不幂等
 *   （曾出现 Identifier 'spaceSum' has already been declared、以及每跑一次生成
 *    产物多出若干行注释）。
 *   之所以要抽取而不是固定写死：引擎状态变量会随改造增减（例如“增量棋型评估”
 *   改造新增了 evalReady / cgFive / boardHashLo / ttGen / spaceSum 等）。若生成器
 *   只输出模板里的旧状态段，就会出现“函数用到新状态变量但没有声明”，
 *   运行时报 xxx is not defined（曾因此让整个引擎不可用）。 */
let boardSize = 19;
