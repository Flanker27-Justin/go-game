...
/* 原先这里钉死数值 42160（= 旧棋型分 10000 + 连接 60 + 中心 1200 + 威胁空间 30000），
 * 一改棋型分值表就失效——属于“版本锁定断言”。改为校验它真正要保证的性质：
 * 活三给正分、比活二更值钱、且棋型分没有被逐格重复计（否则会虚高数倍）。 */
const evThree = ev;
reset();
put(WHITE, [[10, 6], [10, 7]]);              // 一条活二（更弱）
const evTwo = A.evaluateBoard();
checkTrue('一条活三的评估为正', evThree > 0);
checkTrue('活三分数 > 活二分数（棋型价值随连子增长）', evThree > evTwo);
/* “每行只计一次”的精确校验：同样 3 颗白子，摆成“一条活三”带来的棋型分
 * 应当远高于摆成“三个互不相连的孤子”。若实现是逐格计分，两者会接近。
 * （威胁空间项两边都有，量级相近，因此差分能突出棋型分本身的差异。） */
reset();
put(WHITE, [[10, 6], [10, 7], [10, 8]]);
const evLine = A.evaluateBoard();
reset();
put(WHITE, [[4, 4], [12, 12], [16, 3]]);     // 三个互相孤立的白子
const evLoose = A.evaluateBoard();
checkTrue('一条活三 >> 三个孤子（棋型分按线计而非逐格）', evLine > evLoose * 1.3);
...
for (let i = 0; i < 30; i++) {
  reset();
  put(BLACK, [[9, 9]]);
  const m = A.getBestMove(LEVEL_MEDIUM);
